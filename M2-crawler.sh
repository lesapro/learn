...
EOF
    ...
    "2")
        ...
        echow "1. bash M2-crawler.sh -v SITE-MAP-URL    ## To output details to crawler log"
        echow "2. bash M2-crawler.sh -d SITE-URL        ## Debug one URL directly"
        echow "3. bash M2-crawler.sh -b -c -d SITE-URL  ## Debug one URL with cookies and blacklist check"
        ...
        echoB "${EPACE}${EPACE}https://docs.litespeedtech.com/lscache/litemage/crawler/"
        ...
        echow '-d, --debug-url'     
        echo "${EPACE}${EPACE}Test one URL directly. sh M2-crawler.sh -v -d http://example.com/test.html"
        echow '-qs,--crawl-qs'      
