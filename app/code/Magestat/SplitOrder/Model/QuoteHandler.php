...
class QuoteHandler implements QuoteHandlerInterface
{
    ...
    public function normalizeQuotes($quote)
    {
        ...
        foreach ($quote->getAllVisibleItems() as $item) {
            ...
            $product = $this->productRepository->get($item['product']['sku']);
            if (!empty($product->getData($attributes))) {
                $groups[$product->getData($attributes)][] = $item;
            }
        }
